...
for vcf_file in $vcf_files; do
    # Extract the prefix (numbers before .vcf.bgz) from the full path
    prefix=$(basename "$vcf_file" .vcf.bgz)
    
    # Define output filename with _filtered suffix in output directory
    output_file="output/${prefix}_filtered.vcf.bgz"
